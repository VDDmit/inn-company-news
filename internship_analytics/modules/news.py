...
# Жёсткий лимит Яндекса
MAX_QUERY_CHARS = 400


def create_yandex_search_query(user_query: str, domains: List[str]) -> str:
    """
    Оставляем исходный формат для совместимости и логов.
    """
    domain_filters = " | ".join([f"site:{domain}" for domain in domains])
    return f'"{user_query}" ({domain_filters})'


def _batch_domains(user_query: str, domains: List[str]) -> List[str]:
    """
    Режем список доменов на несколько запросов так, чтобы каждый queryText гарантированно
    влезал в лимит 400 символов. Никаких дополнительных ограничений не вводим.

    Формат КАЖДОГО батча строго как в исходном коде:
      "user_query" (site:d1 | site:d2 | ...)

    Простой подход: наращиваем по одному домену, пока длина не приблизится к лимиту.
    """
    # шаблоны вокруг фильтров, сохраняем пробел перед "(" и вокруг "|"
    left = f'"{user_query}" ('
    sep = " | "
    right = ")"
    ...
    batches: List[str] = []
    current: List[str] = []
    current_len = len(left) + len(right)  # базовая длина без фильтров

    for d in domains:
        piece = f"site:{d}"
        add_len = (len(sep) if current else 0) + len(piece)
        if current and current_len + add_len > MAX_QUERY_CHARS:
            # закрываем текущий батч
            batches.append(left + sep.join(f"site:{x}" for x in current) + right)
            current = [d]
            current_len = len(left) + len(right) + len(piece)
        else:
            current.append(d)
            current_len += add_len
...
def get_result_xml(operation_data: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        ...
        logger.info("   ...XML успешно извлечен.")
        return xml_result
    except KeyError:
...
def extract_full_article_text(url: str, domain: str) -> Optional[str]:
    try:
        ...
        if "rbc.ru" in domain:
            ...
            selectors = [{"tag": "div", "class_": "article-body"}]
        elif "tass.ru" in domain:
            selectors = [{"tag": "div", "class_": "text-block"}]
        elif "ria.ru" in domain:
            selectors = [{"tag": "div", "class_": "article__body"}]
        elif "interfax.ru" in domain:
            selectors = [{"tag": "article", "itemprop": "articleBody"}]
...
def run_full_search_and_parse(user_search_query: str, domains_to_search: List[str], num_pages: int,
    ...
    """
    Выполняет полный цикл поиска, парсинга и сохранения новостей.
    ЕСЛИ `domains_to_search` ПУСТОЙ, ВЫПОЛНЯЕТ ОБЫЧНЫЙ ПОИСК ПО ВСЕМУ ИНТЕРНЕТУ.
    Иначе, формирует batched-запросы с фильтрами по доменам.
    """
    all_articles: List[Dict[str, Any]] = []
    queries: List[str] = []

    if not domains_to_search:
        # Если список доменов пуст, выполняем общий поиск
        queries = [f'"{user_search_query}"']
        logger.info(f"Сформирован общий поисковый запрос: {queries[0]}\n")
    else:
        # Старая логика: поиск по списку доменов
        full_query = create_yandex_search_query(user_search_query, domains_to_search)
        logger.info(f"Сформирован поисковый запрос по доменам: {full_query}\n")
        queries = _batch_domains(user_search_query, domains_to_search)
        logger.info(f"Сформировано {len(queries)} запрос(ов) из-за лимитов API.")


    for qidx, q in enumerate(queries, 1):
        logger.info(f"[Запрос {qidx}/{len(queries)}] {q}")
        for page_num in range(num_pages):
            try:
                op_id = start_search_task(q, FOLDER_ID, IAM_TOKEN, page=page_num)
                if op_id:
                    ...
                    if not parsed_page_data:
                        logger.info(f"На странице {page_num} больше нет результатов (запрос {qidx}).")
                        break
    ...
    if not all_articles:
        ...
        # Возвращаем пустую строку вместо None для консистентности
        return ""

    logger.info(
        f"Всего найдено {len(all_articles)} статей. Начинаю извлечение полного текста...")
    ...
    for article in unique_articles:
        ...
        time.sleep(1)
        # Для общего поиска у нас нет парсеров для всех сайтов,
        # поэтому полный текст может и не извлечься. Это нормально.
